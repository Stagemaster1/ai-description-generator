...
// SESSION 4D7 Implementation - Continuous Security Monitoring
// Real-time security monitoring for all endpoints and user activities
...
class SecurityMonitoringSystem {
    constructor() {
        ...
        // SECURITY FIX: Resource management limits to prevent exhaustion attacks
        this.resourceLimits = {
            maxEventEntries: 10000,        // Maximum event count entries
            maxAlertHistory: 1000,         // Maximum alert history entries
            maxThreatLevels: 5000,         // Maximum threat level entries
            maxEventsPerEntry: 100,        // Maximum events per event count entry
            emergencyCleanupThreshold: 0.9 // Trigger emergency cleanup at 90% capacity
        };
    ...
    recordSecurityEvent(eventType, eventData = {}) {
        try {
            // SECURITY FIX: Check resource limits before adding new events
            if (!this.checkResourceLimits()) {
                securityLogger.log('WARN', 'SECURITY_MONITORING_RESOURCE_LIMIT_REACHED', {
                    eventType: eventType,
                    resourceUsage: this.resourceUsage,
                    action: 'event_recording_skipped'
                });
                return; // Skip recording to prevent exhaustion
            }

            const timestamp = Date.now();
            const eventKey = `${eventType}_${eventData.clientIP || 'unknown'}`;
            
            if (!this.eventCounts.has(eventKey)) {
                this.eventCounts.set(eventKey, []);
                this.resourceUsage.eventCountsSize++;
            }
            
            const eventList = this.eventCounts.get(eventKey);
            
            // SECURITY FIX: Limit events per entry to prevent single-key exhaustion
            if (eventList.length >= this.resourceLimits.maxEventsPerEntry) {
            ...
            this.checkAlertThresholds(eventType, eventData);
            
            // Update threat level for this IP/user
            this.updateThreatLevel(eventData.clientIP || eventData.userId || 'unknown', eventType);
            
            // Trigger resource management check if needed
